...
class Diagnostic(object):
    ...
    def gelman_rubin(self, chain=None, threshold=0.05):
        ...
        if chain is None:
            return np.all(
                [
                    self.gelman_rubin(k, threshold=threshold)
                    for k in range(len(self.parent.chains))
                ]
            )
    ...
    def geweke(self, chain=None, first=0.1, last=0.5, threshold=0.05):
        ...
        if chain is None:
            return np.all(
                [
                    self.geweke(k, threshold=threshold)
                    for k in range(len(self.parent.chains))
                ]
            )
        ...
        mean_start = np.array(
            [np.mean(c[:n_start, i]) for c in chains for i in range(c.shape[1])]
        )
        var_start = np.array(
            [
                self._spec(c[:n_start, i]) / c[:n_start, i].size
                for c in chains
                for i in range(c.shape[1])
            ]
        )
        mean_end = np.array(
            [np.mean(c[n_end:, i]) for c in chains for i in range(c.shape[1])]
        )
        var_end = np.array(
            [
                self._spec(c[n_end:, i]) / c[n_end:, i].size
                for c in chains
                for i in range(c.shape[1])
            ]
        )
